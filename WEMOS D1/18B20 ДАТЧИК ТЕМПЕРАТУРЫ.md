{0x28, 0x61, 0x64, 0xA, 0x73, 0x6F, 0xE2, 0xE3} - БЛИЖНИЙ  
{0x28, 0x61, 0x64, 0xA, 0x73, 0xD, 0x30, 0xC3} - ДАЛЬНИЙ  
...
uint8_t s3_addr[] = {0x28, 0xDE, 0xA1, 0x14, 0x0, 0x0, 0x0, 0x1};    //датчик на разъеме
...
void loop() {
  ...
  if (http.begin(client, uurl )) {  // HTTP
    ...
    //Третий датчик пошел :)
    uurl=("http://test:test@grafana.kitrm.ru/data/?ur=");
    uurl+=sensor3.getTemp();  
  if (http.begin(client, uurl )) {  // HTTP
